...
def systems_client(tmp_path, monkeypatch, fake_udl):
    ...
    seed = [{
        "family_id": "test-fam", "family_title": "Test Family", "family_sub": "sub",
        "nation": "RU", "designator": "Test-1", "catalogue_name": "TESTSAT-1",
        "launch_year": 2020, "launch_site": None, "norad_id": "10001",
        "regime": "LEO", "delta_v": None, "status": "onorbit", "life": None,
        "coplanar": None, "notes": "Seed record", "flag": None,
    }]
...
def test_create_requires_token(systems_client):
    payload = {
        "family_id": "f2", "family_title": "F2", "family_sub": "s", "nation": "CN",
        "catalogue_name": "NEWSAT", "launch_year": 2026, "regime": "GEO",
    }
...
def test_create_rejects_malformed_payload(systems_client, auth_headers):
    ...
    response = systems_client.post("/api/systems", json={"nation": "CN"}, headers=auth_headers)
    assert response.status_code == 422


def test_update_requires_token(systems_client):
    seeded_id = systems_client.get("/api/systems").json()["systems"][0]["id"]
    response = systems_client.patch(f"/api/systems/{seeded_id}", json={"status": "decayed"})
    assert response.status_code == 401
...
def test_update_missing_id_404(systems_client, auth_headers):
    response = systems_client.patch("/api/systems/does-not-exist", json={"status": "decayed"}, headers=auth_headers)
    assert response.status_code == 404
...
def test_archive_missing_id_404(systems_client, auth_headers):
    response = systems_client.delete("/api/systems/does-not-exist", headers=auth_headers)
    assert response.status_code == 404
